...
const LoanList: React.FC<LoanListProps> = ({ loans, users = [], onUpdate, onEdit, onDelete, isAdmin, onUserClick }) => {
  ...
  return (
    <div className="space-y-4">
      {loans.map(loan => {
        ...
        return (
          <div key={loan.id} className="bg-white rounded-[24px] shadow-sm border border-gray-100 overflow-hidden transition-all">
            <div 
              className={`p-5 flex justify-between items-center cursor-pointer active:bg-gray-50 ${isExp ? 'bg-gray-50/50' : ''}`}
              onClick={() => setExpandedLoanId(isExp ? null : loan.id)}
            >
              <div className="flex gap-4 items-center">
                <div className="w-12 h-12 bg-bkash-pink/5 rounded-2xl flex items-center justify-center text-bkash-pink">
              ...
              <div className="text-right flex items-center gap-3">
                 ...
                 <svg className={`w-4 h-4 text-gray-300 transition-transform duration-300 ${isExp ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M19 9l-7 7-7-7"></path></svg>
              </div>
            </div>
                     ...
                     <div key={inst.id} className="flex gap-4">
                        <div className="flex flex-col items-center">
                           <div 
                            ...
                            className={`w-5 h-5 rounded-full border-2 transition-all flex items-center justify-center ${isAdmin ? 'cursor-pointer' : 'cursor-default'} ${inst.status === 'Paid' ? 'bg-bkash-pink border-bkash-pink shadow-md' : 'bg-white border-gray-300'}`}
                           >
                             {inst.status === 'Paid' && <svg className="w-3 h-3 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M5 13l4 4L19 7"></path></svg>}
                           </div>
